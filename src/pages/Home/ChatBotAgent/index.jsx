...
const ChatBotAgent = () => {
  ...
  const fetchBotResponse = async (userMessage) => {
    try {
      ...
      return response.data.faqs?.length > 0
        ? response.data.faqs[0].answer
        : "I couldn't find information on that. Can you rephrase?";
    } catch (error) {
  ...
  const updateHistory = (text, role = "model") => {
      ...
      <AnimatePresence>
        {showChatbot && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 w-[calc(100vw-2rem)] sm:w-96 max-w-full bg-white rounded-lg shadow-xl overflow-hidden flex flex-col"
            style={{
          ...
          >
            {/* Header */}
            <div className="bg-pink-500 text-white p-3 md:p-4 flex justify-between items-center">
            ...
            <div
              ref={chatBodyRef}
              className="flex-1 p-3 md:p-4 overflow-y-auto space-y-3 md:space-y-4"
            >
              <div className="flex gap-2 md:gap-3">
                <ChatBotIcon />
                <div className="bg-gray-100 px-3 py-2 md:px-4 md:py-3 rounded-lg rounded-tl-none max-w-[80%]">
...
const ChatMessage = ({ chat }) => {
  return (
    ...
    >
      ...
      <div
        className={`px-3 py-2 md:px-4 md:py-3 rounded-lg max-w-[80%] text-sm md:text-base ${
          chat.role === "user"
  ...
  return (
    ...
    >
      <input
        ...
        className="flex-1 p-2 bg-transparent outline-none text-sm md:text-base"
...
const ChatBotIcon = () => (
  <div className="flex-shrink-0 w-8 h-8 md:w-10 md:h-10 bg-white rounded-full flex items-center justify-center">
    <svg
      ...
      className="w-5 h-5 md:w-6 md:h-6 fill-pink-500"
    >
