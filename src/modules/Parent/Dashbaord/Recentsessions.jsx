    ...
    const getSessionDetails = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter sessions where endDate is less than the current date
          const filteredData = data.filter((item) => new Date(item.endDate) < currentDate && item.playerId === currentUser._id);
  ...
  return (
    <div className="p-4 overflow-x-auto">
      ...
      <div className="pagination mt-4 flex justify-between items-center">
        <div className="flex items-center">
          <button onClick={() => gotoPage(0)} disabled={!canPreviousPage} className="p-2 border mr-2 bg-main-accent rounded-full">
            <FaAngleDoubleLeft />
          </button>
          <button onClick={() => previousPage()} disabled={!canPreviousPage} className="p-1.5 border rounded-full mr-2 bg-main-primary">
            <FaAngleLeft />
          </button>
          <button onClick={() => nextPage()} disabled={!canNextPage} className="p-1.5 border rounded-full mr-2 bg-main-primary">
            <FaAngleRight />
          </button>
          <button onClick={() => gotoPage(pageCount - 1)} disabled={!canNextPage} className="p-2 border mr-2 bg-main-accent rounded-full">
            <FaAngleDoubleRight />
          </button>
        </div>
        <span className="px-4">
